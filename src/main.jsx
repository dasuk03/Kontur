...
const TP_REGISTRY_KEY = 'kontur-tp-registry-v1';
const ONBOARDING_KEY = 'kontur-onboarding-complete-v1';
const TP_SOURCE_PATH = `${import.meta.env.BASE_URL || '/'}map-source.html`;

const TP_STATUS_META = {
  ДСТВ: { label: 'Действующая', tone: 'ok' },
  'ДСТВ КАБН': { label: 'Действующая · кабельная', tone: 'warning' },
  'ДСТВ КАБН ПОТР': { label: 'Действующая · кабельная · потребитель', tone: 'warning' },
  'ДСТВ ПОТР': { label: 'Действующая · потребитель', tone: 'warning' },
  'ДСТВ КАБН ОТАБ ПОТР': { label: 'Действующая · особый статус', tone: 'danger' },
  ОТКЛ: { label: 'Отключена', tone: 'muted' },
  СТРТ: { label: 'Строится', tone: 'muted' },
  ДМНТ: { label: 'Демонтирована', tone: 'danger' },
};

function inferTpType(name = '') {
  const value = name.toUpperCase();
  if (value.includes('ЦРП')) return 'ЦРП';
  if (value.includes('РП')) return 'РП';
  if (value.includes('ЗТП')) return 'ЗТП';
  if (value.includes('СТП')) return 'СТП';
  if (value.includes('БТ-')) return 'БТ';
  if (value.includes('ТПП')) return 'ТПП';
  if (value.includes('КРУН')) return 'КРУН';
  if (value.includes('МТП')) return 'МТП';
  return 'КТП';
}

function normalizeTpRecord(value, index = 0) {
  const source = value && typeof value === 'object' ? value : {};
  const id = String(source.id || `LOCAL-TP-${String(index + 1).padStart(5, '0')}`);
  ...
  return {
    ...
    status: String(source.status || 'ДСТВ'),
    type: source.type || inferTpType(name),
    ...
    tt: source.tt || '',
    ratio: source.ratio || '',
    lightingMeter: source.lightingMeter || '',
    surveyState: source.surveyState || 'Не обследована',
    notes: source.notes || '',
    projectId: source.projectId || '',
    source: source.source || 'RossetiIMDS/map.html',
    updatedAt: source.updatedAt || null,
  };
}

function normalizeTpRegistry(value) {
  return Array.isArray(value) ? value.map(normalizeTpRecord) : [];
}

function parseLegacyTpData(html) {
  const match = String(html || '').match(/const\s+TP_DATA\s*=\s*(\[[\s\S]*?\]);/);
  if (!match) throw new Error('В исходном HTML не найден массив TP_DATA');
  return normalizeTpRegistry(JSON.parse(match[1]));
}

function readLocalTpRegistry() {
  try {
    const raw = localStorage.getItem(TP_REGISTRY_KEY);
    return raw ? normalizeTpRegistry(JSON.parse(raw)) : [];
  } catch {
    return [];
  }
}
...
function tpStatusLabel(status) {
  return TP_STATUS_META[status]?.label || status || 'Без статуса';
}
...
function TPRegistryWorkspace({
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((record) => {
      const haystack = [record.id, record.name, record.status, record.type, record.address, record.meter].join(' ').toLowerCase();
      const queryMatches = !query || haystack.includes(query);
      const statusMatches = statusFilter === 'all' || record.status === statusFilter;
      const surveyMatches = surveyFilter === 'all' || record.surveyState === surveyFilter;
      return queryMatches && statusMatches && surveyMatches;
    });
  }, [records, search, statusFilter, surveyFilter]);
  ...
  const update = (key, value) => setDraft((current) => ({ ...current, [key]: value }));
  const save = () => draft && onSave({ ...draft, updatedAt: new Date().toISOString() });
  const formatCoords = (record) => Number.isFinite(record.lat) && Number.isFinite(record.lon)
  ...
  return <section className="registry-workspace">
    ...
    <div className="registry-grid">
      ...
      <aside className="registry-editor">
        {draft ? <>
          <div className="registry-editor-head"><div><span className="eyebrow">Карточка объекта</span><h2>{draft.name}</h2><p>{draft.id}</p></div><button className="ghost-button" type="button" onClick={() => onSelect(null)} title="Закрыть карточку"><X size={16} /></button></div>
          <div className="registry-editor-scroll">
            <div className="registry-source-note"><FileCode2 size={15} /><span>Источник: {draft.source || 'локальная запись'}</span></div>
            <div className="registry-form-grid"><Field label="ID объекта" value={draft.id} onChange={(value) => update('id', value)} /><Field label="Тип" value={draft.type} onChange={(value) => update('type', value)} /><Field label="Название" value={draft.name} onChange={(value) => update('name', value)} /><Field label="Статус источника" value={draft.status} onChange={(value) => update('status', value)} /><Field label="Широта" type="number" value={draft.lat ?? ''} onChange={(value) => update('lat', value === '' ? null : Number(value))} /><Field label="Долгота" type="number" value={draft.lon ?? ''} onChange={(value) => update('lon', value === '' ? null : Number(value))} /><Field label="Адрес / ориентир" value={draft.address} onChange={(value) => update('address', value)} /><Field label="Состояние обследования" value={draft.surveyState} onChange={(value) => update('surveyState', value)} /></div>
            <div className="registry-form-section"><h3>Паспортные данные</h3><div className="registry-form-grid"><Field label="Трансформатор" value={draft.transformer} onChange={(value) => update('transformer', value)} /><Field label="Мощность, кВА" value={draft.kva} onChange={(value) => update('kva', value)} /><Field label="ПУ" value={draft.meter} onChange={(value) => update('meter', value)} /><Field label="ТТ" value={draft.tt} onChange={(value) => update('tt', value)} /><Field label="Коэффициент трансформации" value={draft.ratio} onChange={(value) => update('ratio', value)} /><Field label="ПУ освещения" value={draft.lightingMeter} onChange={(value) => update('lightingMeter', value)} /></div></div>
            <label className="builder-field full"><span>Заметки обследования</span><textarea value={draft.notes} onChange={(event) => update('notes', event.target.value)} placeholder="Замечания, фото, особенности схемы…" /></label>
            <div className="registry-coords"><MapPin size={15} /><span>{formatCoords(draft)}</span>{Number.isFinite(draft.lat) && Number.isFinite(draft.lon) && <button type="button" onClick={() => window.open(`https://yandex.ru/maps/?ll=${draft.lon}%2C${draft.lat}&z=16&pt=${draft.lon}%2C${draft.lat}`, '_blank', 'noopener,noreferrer')}><ExternalLink size={14} /> Открыть карту</button>}</div>
          </div>
          <div className="panel-footer"><button className="secondary-button" type="button" onClick={() => onOpenProject(draft)}><ArrowRight size={16} /> В рабочий проект</button><button className="primary-button" type="button" onClick={save}><Save size={16} /> Сохранить</button></div>
        </> : <div className="registry-editor-empty"><Database size={28} /><h3>Выберите ТП из списка</h3><p>Карточка справа покажет исходные координаты и позволит внести паспортные данные обследования.</p></div>}
      </aside>
    </div>
  </section>;
}

function Onboarding({ tpCount, onFinish }) {
  const [step, setStep] = useState(0);
  const slides = [
    { icon: <Database size={28} />, title: 'Все ТП — в одном реестре', text: `В «Контур» уже подготовлен локальный справочник из ${tpCount ? tpCount.toLocaleString('ru-RU') : '2 180'} объектов. Ищите по ID, названию, статусу или координатам.` },
    { icon: <MapPin size={28} />, title: 'Сначала найдите нужную ТП', text: 'Откройте раздел «Реестр ТП», найдите объект и проверьте координаты. При необходимости данные можно импортировать повторно из старого map.html.' },
    { icon: <ClipboardCheck size={28} />, title: 'Потом ведите обследование', text: 'Откройте ТП в рабочем проекте, внесите трансформатор, ПУ, ТТ, коэффициент трансформации, замечания и продолжайте работу с фидерами, опорами и схемой.' },
  ];
  const current = slides[step];
  return <div className="onboarding-backdrop"><section className="onboarding-card" role="dialog" aria-modal="true" aria-labelledby="onboarding-title"><div className="onboarding-brand"><div className="brand-mark"><Zap size={16} fill="currentColor" /></div><span>КОНТУР</span><span className="onboarding-label">Быстрый старт</span></div><div className="onboarding-illustration">{current.icon}</div><span className="eyebrow">Шаг {step + 1} из {slides.length}</span><h2 id="onboarding-title">{current.title}</h2><p>{current.text}</p><div className="onboarding-dots">{slides.map((_, index) => <i key={index} className={index === step ? 'active' : ''} />)}</div><div className="onboarding-actions">{step > 0 ? <button className="secondary-button" type="button" onClick={() => setStep((value) => value - 1)}><ArrowLeft size={15} /> Назад</button> : <button className="text-button" type="button" onClick={onFinish}>Пропустить</button>}{step < slides.length - 1 ? <button className="primary-button" type="button" onClick={() => setStep((value) => value + 1)}>Далее <ArrowRight size={15} /></button> : <button className="primary-button" type="button" onClick={onFinish}>Открыть реестр <ArrowRight size={15} /></button>}</div></section></div>;
}
...
function App() {
  ...
  useEffect(() => {
    let cancelled = false;
    const local = readLocalTpRegistry();
    if (local.length) {
      setTpRegistry(local);
      setTpRegistryLoading(false);
      return () => { cancelled = true; };
    }
    fetch(TP_SOURCE_PATH)
      .then((response) => {
        if (!response.ok) throw new Error('Источник карты недоступен');
        return response.text();
      })
      .then((html) => {
        if (cancelled) return;
        const imported = parseLegacyTpData(html);
        setTpRegistry(imported);
        localStorage.setItem(TP_REGISTRY_KEY, JSON.stringify(imported));
      })
      .catch(() => {
        if (!cancelled) window.setTimeout(() => window.alert('Не удалось автоматически загрузить старый реестр map.html. Используйте кнопку «Импорт map.html» в разделе «Реестр ТП».'), 300);
      })
      .finally(() => { if (!cancelled) setTpRegistryLoading(false); });
    return () => { cancelled = true; };
  }, []);
  ...
  const navigate = (tab) => {
    ...
    else if (tab === 'registry') setOpenPanel(null);
  ...
  const importData = (event) => {
    ...
    reader.onload = () => {
      try {
        ...
        if (Array.isArray(payload.tpRegistry)) {
          const nextRegistry = normalizeTpRegistry(payload.tpRegistry);
          setTpRegistry(nextRegistry);
          localStorage.setItem(TP_REGISTRY_KEY, JSON.stringify(nextRegistry));
        }
        setSelectedTab('map');
        setOpenPanel(null);
      } catch {
  ...
  const mergeTpRegistry = (incoming) => {
    const normalized = normalizeTpRegistry(incoming);
    setTpRegistry((current) => {
      const byId = new Map(current.map((record) => [record.id, record]));
      normalized.forEach((record) => byId.set(record.id, { ...byId.get(record.id), ...record }));
      const next = [...byId.values()];
      localStorage.setItem(TP_REGISTRY_KEY, JSON.stringify(next));
      return next;
    });
    return normalized.length;
  };
  const importTpRegistry = (event) => {
    const file = event.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => {
      try {
        ...
        const parsed = file.name.toLowerCase().endsWith('.json')
          ? JSON.parse(text)
          : parseLegacyTpData(text);
        const incoming = Array.isArray(parsed) ? parsed : (parsed.tpRegistry || parsed.records || parsed.data || []);
        const count = mergeTpRegistry(incoming);
        window.alert(`Импорт завершён. Обработано объектов: ${count}.`);
      } catch (error) {
        window.alert(`Не удалось импортировать реестр: ${error.message || 'проверьте формат файла'}.`);
      }
    };
    reader.readAsText(file);
    event.target.value = '';
  };
  const reloadTpSource = async () => {
    setTpRegistryLoading(true);
    try {
      const response = await fetch(`${TP_SOURCE_PATH}?v=${Date.now()}`);
      if (!response.ok) throw new Error('Источник недоступен');
      const imported = parseLegacyTpData(await response.text());
      mergeTpRegistry(imported);
      window.alert(`Источник обновлён. Обработано объектов: ${imported.length}.`);
    } catch (error) {
      window.alert(`Не удалось обновить источник: ${error.message || 'ошибка загрузки'}.`);
    } finally {
      setTpRegistryLoading(false);
    }
  };
  const saveTpRecord = (record) => {
    setTpRegistry((current) => {
      const oldId = selectedTpId;
      const replaced = current.some((item) => item.id === oldId)
        ? current.map((item) => item.id === oldId ? normalizeTpRecord(record) : item)
        : [...current, normalizeTpRecord(record)];
      localStorage.setItem(TP_REGISTRY_KEY, JSON.stringify(replaced));
      return replaced;
    });
    setSelectedTpId(record.id);
  };
  const createTpRecord = () => {
    const record = normalizeTpRecord({ id: `LOCAL-TP-${Date.now()}`, name: 'Новая ТП', status: 'ДСТВ', type: 'КТП', surveyState: 'В работе', source: 'Создано в Контуре' });
    setTpRegistry((current) => {
  ...
  const exportTpRegistry = () => saveBlob('kontur-tp-registry.json', JSON.stringify({ exportedAt: new Date().toISOString(), source: 'Контур', tpRegistry }, null, 2), 'application/json;charset=utf-8');
  const openTpInProject = (record) => {
    const nextProject = normalizeProject({ ...project, id: `survey-${record.id.toLowerCase()}`, settlement: record.address || project.settlement, ktp: { ...project.ktp, id: record.id, name: record.name, address: record.address || record.name, coords: [record.lat, record.lon], status: record.surveyState, transformer: record.transformer, kva: record.kva, meter: record.meter, tt: record.tt, ratio: record.ratio, lightingMeter: record.lightingMeter } });
    setProject(nextProject);
    localStorage.setItem('kontur-survey-project', JSON.stringify(nextProject));
    setSelectedTab('map');
    setOpenPanel('ktp');
  ...
  const syncLocal = () => {
    ...
    localStorage.setItem(TP_REGISTRY_KEY, JSON.stringify(tpRegistry));
  ...
  return <div className="app-shell">
    ...
    <div className="workspace">
      ...
      <nav className={`sidebar ${showMobileNav ? 'sidebar-open' : ''}`}>
        ...
        <button className={`nav-item ${selectedTab === 'registry' ? 'active' : ''}`} type="button" onClick={() => navigate('registry')}><Database size={17} /><span>Реестр всех ТП</span><b>{tpRegistry.length ? tpRegistry.length.toLocaleString('ru-RU') : '…'}</b></button>
